...
class FFmpegStreamerApp:
    ...
    def create_config_widgets(self):
        ...
        self.output_type_combo = ttk.Combobox(output_group, textvariable=self.output_type_var, values=["UDP", "SRT"], state='readonly', width=15)
        ...
        ttk.Label(output_group, text="Video Bitrate (kbps):").grid(row=2, column=0, padx=5, pady=5, sticky=W)
        ...
        self.video_bitrate_entry.grid(row=2, column=1, padx=5, pady=5, sticky=W)
    ...
    def on_output_type_change(self, event=None):
        """Handles changes in the output type selection. (Currently no complex logic needed)."""
        pass
    ...
    def save_current_config_to_memory(self):
        ...
        config["srt_mode"] = self.srt_mode_var.get()
    ...
    def get_output_url(self, config):
        ...
        if config['output_type'] == "UDP":
            return f"udp://@{config['output_ip']}:{config['output_port']}"
        elif config['output_type'] == "SRT":
            return f"srt://{config['output_ip']}:{config['output_port']}?mode=caller"
        return ""
