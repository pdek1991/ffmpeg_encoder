...
import sys # For _MEIPASS in executable path handling
import struct # For UDP multicast membership
...
class CustomTooltip:
    ...
    def schedule(self):
        """Schedules the tooltip to show after a delay."""
        self.unschedule()
    ...
    def show(self):
        ...
        label = ttk.Label(tw, text=self.text, background="#ffffe0", relief="solid",
...
# --- Modern UI Palette and Helpers ---
MODERN_PALETTE = {
    "bg": "#1e1f29",            # Dark matte
    "card_bg": "#2a2f40",       # Solid dark card color
    "sidebar_bg": "#2a2f40",    # Matches card
    "logo_bg": "#1b1f2e",       # Dark logo section
    "accent": "#3A8DFF",        # Bright blue
    "panel_bg": "#1e1f29",      # Light soft panel
    "row_glow": "#66c2ff",      # Glow color
    "danger": "#FF5E6C",        # Alert red
    "warning": "#FFD247",       # Warning yellow
    "success": "#22C88B",       # Success green
    "text_main": "#ECEFF4",     # Main text
    "text_dim": "#8C98A9",      # Dimmed text
    "divider": "#343a4d",       # Divider
    "button_fg": "#ffffff"      # Button foreground
}

def _set_global_fonts(root):
    """Sets global font options for Tkinter and ttk widgets."""
    try:
        # Define font objects
        default_font = tkinter.font.Font(family="Segoe UI", size=12)
        combobox_listbox_font = tkinter.font.Font(family="Segoe UI", size=11)
        entry_font = tkinter.font.Font(family="Segoe UI", size=12)
        label_font = tkinter.font.Font(family="Segoe UI", size=12)

        root.option_add("*Font", default_font)
        root.option_add("*TCombobox*Listbox*Font", combobox_listbox_font)
        root.option_add("*Entry.Font", entry_font)
        root.option_add("*TEntry.Font", entry_font) # ttk.Entry
        root.option_add("*Label.Font", label_font) # tk.Label
        root.option_add("*TLabel.Font", label_font) # ttk.Label
    except Exception as e:
        print(f"Warning: Could not set global fonts. Using system defaults. Error: {e}") # Fallback if font is not available

def _modernize_style():
    """Applies modern ttkbootstrap styles and custom configurations."""
    style = ttkb.Style(theme="darkly") # Use the 'darkly' theme as specified

    # Define font objects for specific styles
    sidebar_header_font = tkinter.font.Font(family="Segoe UI", size=15, weight="bold")
    logo_text_font = tkinter.font.Font(family="Segoe UI", size=16, weight="bold")
    global system_meter_label_font
    system_meter_label_font = tkinter.font.Font(family="Segoe UI", size=9, weight="bold")
    global stream_status_label_font
    stream_status_label_font = tkinter.font.Font(family="Segoe UI Semibold", size=13)


    # General configuration for all widgets
    style.configure('.', background=MODERN_PALETTE["bg"], foreground=MODERN_PALETTE["text_main"], font=tkinter.font.Font(family="Segoe UI", size=12))

    # Custom styles for Frames and LabelFrames to match the palette
    style.configure('ModernCard.TLabelframe', background=MODERN_PALETTE["panel_bg"], font=tkinter.font.Font(family="Segoe UI", size=15, weight="bold"),
                    borderwidth=0, foreground=MODERN_PALETTE["accent"])
    style.configure('ModernCard.TLabelframe.Label', foreground=MODERN_PALETTE["accent"], background=MODERN_PALETTE["panel_bg"])
    style.configure('ModernLabelframe.TLabelframe', background=MODERN_PALETTE["panel_bg"])
    style.configure("ModernCardInner.TFrame", background=MODERN_PALETTE["panel_bg"])
    style.configure("TFrame", background=MODERN_PALETTE["bg"]) # Default TFrame background

    # Custom style for sidebar header
    style.configure("SidebarHeader.TLabel", background=MODERN_PALETTE["sidebar_bg"], foreground=MODERN_PALETTE["text_main"], font=sidebar_header_font)

    # Custom style for logo panel label
    style.configure("LogoPanel.TLabel", background=MODERN_PALETTE["logo_bg"])

    # Entry widgets
    style.configure('ModernEntry.TEntry', fieldbackground=MODERN_PALETTE["card_bg"], background=MODERN_PALETTE["card_bg"],
                    foreground=MODERN_PALETTE["text_main"], borderwidth=1, relief="flat", highlightbackground=MODERN_PALETTE["divider"], highlightthickness=1)
    style.map('ModernEntry.TEntry', fieldbackground=[('focus', '#333A48')])

    # Button styles (bootstyles are often sufficient, but custom can be made)
    style.configure('Accent.TButton', background=MODERN_PALETTE["accent"], foreground="#fff", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"))
    style.map('Accent.TButton', background=[('active', '#5A9AE2')])
    style.configure('Success.TButton', background=MODERN_PALETTE["success"], foreground="#fff", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"))
    style.map('Success.TButton', background=[('active', '#2ED881')])
    style.configure('Danger.TButton', background=MODERN_PALETTE["danger"], foreground="#fff", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"))
    style.map('Danger.TButton', background=[('active', '#FF7483')])

    # Channel pill buttons (specific styles)
    style.configure('ModernChannel.TButton', foreground=MODERN_PALETTE["text_main"], background=MODERN_PALETTE["sidebar_bg"],
                    relief="flat", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"), padding=9, borderwidth=0)
    style.map('ModernChannel.TButton', background=[('active', MODERN_PALETTE["accent"]), ('!disabled', MODERN_PALETTE["sidebar_bg"])])

    style.configure('ModernChannelDanger.TButton', foreground=MODERN_PALETTE["text_main"], background=MODERN_PALETTE["danger"],
                    relief="flat", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"), padding=9, borderwidth=0)
    style.map('ModernChannelDanger.TButton', background=[('active', MODERN_PALETTE["danger"])])

    style.configure('ModernChannelSuccess.TButton', foreground=MODERN_PALETTE["text_main"], background=MODERN_PALETTE["success"],
                    relief="flat", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"), padding=9, borderwidth=0)
    style.map('ModernChannelSuccess.TButton', background=[('active', MODERN_PALETTE["success"])])

    style.configure('ModernChannelActive.TButton', foreground="#fff", background=MODERN_PALETTE["accent"],
                    relief="flat", font=tkinter.font.Font(family="Segoe UI", size=12, weight="bold"), padding=9, borderwidth=0)
    style.map('ModernChannelActive.TButton', background=[('active', MODERN_PALETTE["accent"])])

    # Progressbar styles
    style.configure("TProgressbar", background=MODERN_PALETTE["accent"]) # Default progressbar color

    # Label (ttk.Label) specific styles
    style.configure("TLabel", background=MODERN_PALETTE["panel_bg"], foreground=MODERN_PALETTE["text_main"]) # Default for labels on panel_bg

    # Combobox styles
    style.configure("TCombobox", fieldbackground=MODERN_PALETTE["card_bg"], background=MODERN_PALETTE["card_bg"],
                    foreground=MODERN_PALETTE["text_main"], selectbackground=MODERN_PALETTE["accent"],
                    selectforeground="#fff", borderwidth=1, relief="flat", highlightbackground=MODERN_PALETTE["divider"], highlightthickness=1)
    style.map("TCombobox", fieldbackground=[('readonly', MODERN_PALETTE["card_bg"])])

    # Separator style
    style.configure("ModernDivider.TSeparator", background=MODERN_PALETTE["divider"])


def modern_status_indicator(parent, color, glow=False):
    """Creates a small circular status indicator canvas."""
    # The canvas itself uses 'bg' directly, and it should match the parent's background
    c = tk.Canvas(parent, width=20, height=20, highlightthickness=0, bg=parent.cget('background'))
    if glow:
        c.create_oval(2,2,18,18, fill=color, outline=color)
        c.create_oval(6,6,14,14, fill="black", outline="", stipple="gray25")
    else:
        c.create_oval(4,4,16,16, fill=color, outline="#222")
    c.pack(side="left", padx=3)
    return c

def pill_channel_button(parent, text, style, command):
    """Creates a styled 'pill' shaped button for channels."""
    btn = ttk.Button(parent, text="    " + text + "    ", style=style, command=command)
    btn.pack(fill="x", pady=(2,4), padx=0, ipadx=2)
    return btn

def modern_card(parent, text):
    """Creates a ttk.LabelFrame styled as a modern card."""
    return ttk.LabelFrame(parent, text=text, style="ModernCard.TLabelframe", padding=(18,14,18,14))
...
class FFmpegStreamerApp:
    def __init__(self, master):
        ...
        _modernize_style() # Apply modern styles first
        _set_global_fonts(self.master) # Set global fonts
        self.master.configure(bg=MODERN_PALETTE["bg"]) # Set root window background
        ...
        main_frame = ttk.Frame(self.master, padding="15", style='TFrame') # Increased padding, added style
        ...
        # Changed to ttk.Label and used custom style
        ttk.Label(left_frame, text="Channels", style="SidebarHeader.TLabel").pack(pady=15) # Larger font, primary color

        self.channel_list_frame = ttk.Frame(left_frame, style='TFrame') # Added style
        ...
        right_frame = ttk.Frame(main_frame, style='TFrame') # Added style
        right_frame.pack(side=RIGHT, fill=BOTH, expand=True)

        # Create a canvas and scrollbar for the right frame
        self.canvas = tk.Canvas(right_frame, highlightthickness=0, bg=MODERN_PALETTE["panel_bg"]) # Set canvas background
        self.scrollbar = ttk.Scrollbar(right_frame, orient="vertical", command=self.canvas.yview)
        self.scrollable_frame = ttk.Frame(self.canvas, style='ModernCardInner.TFrame') # Use ModernCardInner.TFrame style
        ...
        top_bar_frame = ttk.Frame(self.scrollable_frame, style='ModernCardInner.TFrame') # Use ModernCardInner.TFrame style
        top_bar_frame.pack(fill=X, pady=(0, 15)) # Increased pady

        # Changed to ttk.Frame and used ModernCardInner.TFrame style
        status_frame = ttk.Frame(top_bar_frame, style='ModernCardInner.TFrame')
        status_frame.pack(side=LEFT, anchor='w', padx=(0, 20)) # Added padx
        
        # Changed to ttk.Label and removed bg/fg, relying on TLabel style
        ttk.Label(status_frame, text="Stream Status:", font=stream_status_label_font).pack(side=LEFT, padx=(0,10)) # Increased font size
        
        self.status_indicators_frame = ttk.Frame(status_frame, style='ModernCardInner.TFrame') # Added style
        ...
        self.system_meters_frame = ttk.Frame(top_bar_frame, style='ModernCardInner.TFrame') # Added style
        ...
        # CPU Progress Bar
        cpu_pb_frame = ttk.Frame(self.system_meters_frame, style='ModernCardInner.TFrame') # Added style
        ...
        self.cpu_pb_label = ttk.Label(cpu_pb_frame, text="CPU", font=system_meter_label_font) # Changed font to Segoe UI
        self.cpu_pb_label.pack(side=BOTTOM)

        # RAM Progress Bar
        ram_pb_frame = ttk.Frame(self.system_meters_frame, style='ModernCardInner.TFrame') # Added style
        ...
        self.ram_pb_label = ttk.Label(ram_pb_frame, text="RAM", font=system_meter_label_font) # Changed font to Segoe UI
        self.ram_pb_label.pack(side=BOTTOM)

        # Network Progress Bar
        network_pb_frame = ttk.Frame(self.system_meters_frame, style='ModernCardInner.TFrame') # Added style
        ...
        self.network_pb_label = ttk.Label(network_pb_frame, text="NW", font=system_meter_label_font) # Changed font to Segoe UI
        self.network_pb_label.pack(side=BOTTOM)

        # Changed to ttk.Label and used custom style
        self.logo_label = ttk.Label(top_bar_frame, style="LogoPanel.TLabel")
        ...
        # Configuration Tab
        self.config_tab = ttk.Frame(self.notebook, padding="20", style='TFrame') # Increased padding, added style
    ...
    def create_status_indicators(self):
        ...
        for channel_name in self.channels.keys():
            # Canvas background matches parent's background (which is ModernCardInner.TFrame)
            canvas = tk.Canvas(self.status_indicators_frame, width=20, height=20,
                   bg=MODERN_PALETTE["panel_bg"], highlightthickness=0, relief=tk.RIDGE, bd=1)
    ...
    def load_logo(self):
        ...
        try:
            ...
            # Adjust path for executable if needed
            ...
            if os.path.exists(full_logo_path):
                ...
                self.logger.warning(f"Logo file '{full_logo_path}' not found. Using default text.")
                self.logo_label.config(text="VigilSiddhi", font=logo_text_font, foreground=MODERN_PALETTE["text_main"], background=MODERN_PALETTE["logo_bg"])
        except Exception as e:
    ...
    def create_config_widgets(self):
        ...
        self.display_name_entry = ttk.Entry(self.input_group, textvariable=self.display_name_var, style="ModernEntry.TEntry") # Added style
        ...
        self.input_ip_port_frame = ttk.Frame(self.input_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.input_ip_port_frame, textvariable=self.input_ip_var, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky='we') # Added style
        ttk.Label(self.input_ip_port_frame, text="Port:").grid(row=0, column=2, padx=5, pady=2, sticky=W)
        self.input_port_var = tk.StringVar()
        ttk.Entry(self.input_ip_port_frame, textvariable=self.input_port_var, width=10, style="ModernEntry.TEntry").grid(row=0, column=3, padx=5, pady=2, sticky=W) # Added style
        
        self.input_url_frame = ttk.Frame(self.input_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.input_url_frame, textvariable=self.input_url_var, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky='we') # Added style

        self.srt_mode_frame = ttk.Frame(self.input_group, style='ModernCardInner.TFrame') # Added style
        ...
        self.local_bind_interface_frame = ttk.Frame(self.input_group, style='ModernCardInner.TFrame') # Added style
        ...
        self.output_ip_port_frame = ttk.Frame(output_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.output_ip_port_frame, textvariable=self.output_ip_var, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky='we') # Added style
        ...
        ttk.Entry(self.output_ip_port_frame, textvariable=self.output_port_var, width=10, style="ModernEntry.TEntry").grid(row=0, column=3, padx=5, pady=2, sticky=W) # Added style

        self.output_url_frame = ttk.Frame(output_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.output_url_frame, textvariable=self.output_url_var, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky='we') # Added style

        self.output_srt_mode_frame = ttk.Frame(output_group, style='ModernCardInner.TFrame') # Added style
        ...
        self.output_rtp_protocol_frame = ttk.Frame(output_group, style='ModernCardInner.TFrame') # Added style
        ...
        self.output_rtp_payload_type_frame = ttk.Frame(output_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.output_rtp_payload_type_frame, textvariable=self.output_rtp_payload_type_var, width=10, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky=W) # Added style

        self.video_bitrate_label = ttk.Label(output_group, text="Video Bitrate (kbps):")
        self.video_bitrate_var = tk.StringVar(value="2000")
        self.video_bitrate_entry = ttk.Entry(output_group, textvariable=self.video_bitrate_var, width=15, style="ModernEntry.TEntry") # Added style
        ...
        ttk.Entry(self.advanced_options_group, textvariable=self.input_probesize_var, width=15, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        ttk.Entry(self.advanced_options_group, textvariable=self.input_analyzeduration_var, width=15, style="ModernEntry.TEntry").grid(row=1, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        ttk.Entry(self.advanced_options_group, textvariable=self.output_max_delay_var, width=15, style="ModernEntry.TEntry").grid(row=2, column=1, padx=5, pady=2, sticky=W) # Added style
        ttk.Label(self.advanced_options_group, text="(0 for default)").grid(row=2, column=2, padx=2, pady=2, sticky=W)

        self.srt_output_options_frame = ttk.Frame(self.advanced_options_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.srt_output_options_frame, textvariable=self.output_srt_latency_var, width=10, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        ttk.Entry(self.srt_output_options_frame, textvariable=self.output_srt_maxbw_var, width=10, style="ModernEntry.TEntry").grid(row=1, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        ttk.Entry(self.srt_output_options_frame, textvariable=self.output_srt_sndbuf_var, width=10, style="ModernEntry.TEntry").grid(row=3, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        ttk.Entry(self.srt_output_options_frame, textvariable=self.output_srt_rcvbuf_var, width=10, style="ModernEntry.TEntry").grid(row=4, column=1, padx=5, pady=2, sticky=W) # Added style
        ttk.Label(self.srt_output_options_frame, text="(0 for default)").grid(row=4, column=2, padx=2, pady=2, sticky=W)

        self.udp_output_options_frame = ttk.Frame(self.advanced_options_group, style='ModernCardInner.TFrame') # Added style
        ...
        ttk.Entry(self.udp_output_options_frame, textvariable=self.output_udp_pkt_size_var, width=10, style="ModernEntry.TEntry").grid(row=0, column=1, padx=5, pady=2, sticky=W) # Added style
        ...
        self.action_buttons_frame = ttk.Frame(self.config_tab, padding="15", style='TFrame') # Increased padding, added style
    ...
    def update_status_indicators(self):
        ...
        for name, channel_data in self.channels.items():
            ...
            if name in self.status_indicators:
                # Update canvas color. Canvas uses 'bg' directly.
    ...
    def _start_preview_internal(self, preview_type):
        ...
        try:
            ...
            self.logger.error("ffplay executable not found. Remedy: Install FFmpeg and ensure its executable directory is in your PATH environmental variable.")
